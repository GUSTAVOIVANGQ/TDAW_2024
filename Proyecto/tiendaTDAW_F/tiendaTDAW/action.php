...
include "db.php";
// Consulta todas las categorías de la base de datos y las muestra junto con la cantidad de productos en cada categoría.
if(isset($_POST["category"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		...
		while($row = mysqli_fetch_array($run_query)){ 
			$cid = $row["cat_id"];
			$cat_name = ucwords(strtolower($row["cat_title"]));
                        	...
                        	<div class='col-*-* col-xs-6' >
								<a href='product.php?p=$pro_id'><div class='product'>
									<div class='product-img'>
										...
										";
										if($pro_stock == 0){
										 echo "<div class='texto-superpuesto'>Producto sin stock</div>";
										}
									echo"										
									</div></a>
...
if (isset($_POST["addToCart"])) {
	...
	$stock_query = "SELECT stock FROM products WHERE product_id = '$p_id'";
			...
			echo '
				<script>
					Swal.fire({
						...
						text: "El producto ya se encuentra en el carrito, ¿qué deseas hacer?",
						...
						if (result.isConfirmed) {
							...
							window.location.href = "cart.php"; // Redirige a cart.php al cancelar
						}
					});
				</script>
			';
				...
				echo '
					<script>
						Swal.fire({
							...
							text: "Producto añadido al carrito, ¿qué deseas hacer?",
							showCancelButton: true,
							confirmButtonText: "Seguir comprando",
							cancelButtonText: "Ir al carrito"
						}).then((result) => {
...
//Aqui se cuentan los articulos de los articulos del usuario
if (isset($_POST["count_item"])) {
	//Cuando el usuario inicia sesión, contaremos la cantidad de artículos en el carrito utilizando 
	//la identificación de sesión del usuario
	if (isset($_SESSION["uid"])) {
		$sql = "SELECT COUNT(*) AS count_item FROM cart WHERE user_id = $_SESSION[uid]";
	}else{
		//Cuando el usuario no ha iniciado sesión, contaremos la cantidad de artículos en el carrito utilizando 
		//la dirección IP única del usuario.
...
//Obtener artículo del carrito de la base de datos al menú desplegable
if (isset($_POST["Common"])) {

	if (isset($_SESSION["uid"])) {
		//Cuando el usuario inicie sesión, esta consulta se ejecutará
		...
		//Cuando el usuario no haya iniciado sesión, esta consulta se ejecutará
	...
	if (isset($_POST["getCartItem"])) {
		//muestra el artículo del carrito en el menú desplegable
		if (mysqli_num_rows($query) > 0) {
			$n=0;
			$total_price=0;
    ...
    if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			//muestra el artículo del carrito del usuario con el botón "Listo para pagar" si el usuario no ha iniciado sesión
			echo '<div class="main ">
			...
			<form method="post" action="login_form.php">
				...
				if (!isset($_SESSION["uid"])) {
					echo '
							...
							<a href="" data-toggle="modal" data-target="#Modal_register" class="btn btn-success">Listo para pagar</a></td>
								</tr>
							</tfoot>
